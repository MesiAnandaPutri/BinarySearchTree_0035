...
public:
	...
	void insert(string element) // inser a node in the binary search tree
	{
		...
		if (element < parent->info) // if the value in the data field of the new node is less then that od the parent
		{
			parent->leftchild = newNode; // Make the left child of the parrent points to the new node
		}
		else if (element > parent->info) // if the value in the data field of the new data is greater than 
		{
			parent->rightchild = newNode; // Make the right child of the parrent points to the new node
		}


		void search(string element, Node * &parent, Node * &currentNode)
		{
			// this function searchhes the currentNode of the specified Node as well as the ncurrent node of its parent
			currentNode = ROOT;
		}
	}
};